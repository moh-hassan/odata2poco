...
if (Get-Process -Name "mitmdump" -ErrorAction SilentlyContinue) {
    ...
    $process = Start-Process -FilePath "$folderPath\mitmdump.exe" -ArgumentList "-p $port --proxyauth `"user:password`" --set block_global=false" -WindowStyle minimized -PassThru
