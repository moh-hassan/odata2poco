...
namespace OData2Poco
{
    ...
    {
        private readonly IDictionary<string, ClassTemplate> _model; //= new Dictionary<string, ClassTemplate>();
        ...
        public List<ClassTemplate> ClassList
        {
            get
            {
                return _model.Select(kvp => kvp.Value).ToList();
            }
...
";
            ...
            // .WriteLineComment("uncomment for spatial data type and run Install-Package System.Spatial ")
            // .WriteLineComment("using System.Spatial;");
            var assemplyManager = new AssemplyManager(PocoSetting, _model);
        ...
        public PocoClassGeneratorCs(IPocoGenerator pocoGen, PocoSetting setting = null)
        {
            ...
            _pocoGen = pocoGen;
            _model = new Dictionary<string, ClassTemplate>();
        ...
        public string GeneratePoco()
        {
            ...
            //var json = JsonConvert.SerializeObject(_classDictionary, Formatting.Indented);
        ...
        internal string ClassToString(ClassTemplate ent, bool includeNamespace = false)
        {
            ...
            //v1.4
            //add TableAttribute for class that only has EntitySetName
            //if (PocoSetting.AddTableAttribute)
            //{
         ...
         //   csTemplate.StartClass(ent.Name, PocoSetting.Inherit, partial:true); //delayed to a future release to avoid change of most test cases
            foreach (var p in ent.Properties)
            {
                ...
                //generate attributes
                //foreach (var item in pp.GetAttributes(PocoSetting))
                //{
                ...
                //if (!string.IsNullOrEmpty( pp.KeyAttribute)) csTemplate.WriteLine(pp.KeyAttribute);
                //if (!string.IsNullOrEmpty(pp.RequiredAttribute)) csTemplate.WriteLine(pp.RequiredAttribute);
                //if (!string.IsNullOrEmpty(pp.JsonAttribute)) csTemplate.WriteLine(pp.JsonAttribute);


                //csTemplate.WriteLine(pp.ToString());
                ...
                //v2.2.0
                //convert p.PropName to PascalCase/CamelCase with jsonPropertyAttribute
                //example:     [JsonProperty(PropertyName = "email")]
                //public string Email { get; set; }



                //csTemplate.WriteLineProperty(p.PropType + nulType, pp.Name, comment: p.PropComment, isVirtual: virtualprop);
               
                //csTemplate.WriteLine(pp.PropertyDeclaration);
